...
{
    public class ConnectWithMapAndMoveTowardsCamera : MonoBehaviour
    {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began)
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    if (hit.collider.gameObject == gameObject)
                    {
                        ...
                        else if(touchIndex % 3 == 2)
                        {
                            StartCoroutine(MoveToObjectPosition(destinationObject1.transform.position,0f));
                        }
                        ...
                        else
                        {
                            ...
                            StartCoroutine(MoveObjectToOriginalPosition());
                        }
        ...
        IEnumerator MoveToObjectPosition(Vector3 targetPosition, float delay)
        {
            yield return new WaitForSeconds(delay);
